...
export function VideoBGEffectModal({ open, setOpen }): ReactElement {
	...
	useEffect(() => {
		if (open) {
			...
			const bgInstance = new VideoProcessor(videoEl.current, canvasEl.current, {
				blurValue: Number(blurAmount),
				aspectRatio: cropMap[currentCropRatio],
			});
	...
	const buttonClass = 'px-4 py-2 rounded-md shadow-md transition-all duration-200 ease-in-out font-medium';
	...
	const inputRangeClass = 'w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-blue-600';
	...
	const imageOptionClass = (isSelected) =>
		`w-20 h-16 rounded-md overflow-hidden cursor-pointer border-2 ${
			isSelected ? 'border-blue-500 ring-2 ring-blue-500' : 'border-gray-300 hover:border-blue-300'
							...
							<div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50">
								<div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto flex flex-col md:flex-row gap-6">
									...
									<div className="flex-1 flex flex-col items-center justify-center bg-gray-800 rounded-lg p-2 min-h-[300px] md:min-h-[400px] relative overflow-hidden">
										...
										<div
											style={{
												paddingTop: getPaddingTopForRatio(),
											}}
										...
										>
											<div className="absolute inset-0 flex items-center justify-center flex-col text-white text-lg font-semibold p-4 text-center">
												...
												<p className="text-sm text-gray-400 mt-2">
													Current Crop: {currentCropRatio} | Blur: {blurAmount}px |
													Background:{' '}
													{selectedBackground === 'none' ? 'None' : selectedBackground}
												</p>
											</div>
										</div>
									</div>
									...
									<div className="w-full md:w-80 flex flex-col gap-5">
										...
										{/* Crop Ratios */}
										<div>
											<label className={labelClass}>Crop Ratio</label>
												...
												{['16:9', '4:3', '1:1'].map((ratio) => (
													<button
														...
														onClick={() => {
															...
															bgRef.current.updateOptions({
																aspectRatio: cropMap[ratio],
															});
														}}
														className={optionButtonClass(currentCropRatio === ratio)}
													>
														{ratio}
													</button>
												))}
										...
										{/* Background Blur */}
										<div>
											<label htmlFor="blurRange" className={labelClass}>
												Background Blur
											</label>
											<input
												type="range"
											...
											/>
											<span className="text-sm text-gray-500 mt-1 block">{blurAmount}px</span>
										</div>
										...
										<div>
											...
											<div className="grid grid-cols-3 gap-3">
												<div
													...
													className={
														imageOptionClass(selectedBackground === 'none') +
												...
												{Object.keys(backgroundImages).map((key) => (
													<div
														...
														className={imageOptionClass(selectedBackground === key)}
													>
														<img
										...
										<div className="mt-auto pt-4 border-t">
											<button onClick={handleClose} className={dangerButtonClass + ' w-full'}>
												Close UI
											</button>
										</div>
									</div>
